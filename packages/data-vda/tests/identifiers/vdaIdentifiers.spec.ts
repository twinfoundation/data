...
import { VdaIdentifiers } from "../../src/identifiers/vdaIdentifiers";
...
describe("VdaIdentifiers", () => {
	test("Test should successfully extract VDA Object Identifier", async () => {
		const vdaIdentifiers: IVdaObjectIdentifier | undefined = VdaIdentifiers.extractIdentifier(
	...
	test("Test should successfully extract RFID read point VDA URN", async () => {
		const readPoint: IVdaReadPoint | undefined = VdaIdentifiers.extractReadPoint(
	...
	test("Test should successfully extract Biz Location URN", async () => {
		const bizLocation: IVdaBizLocation | undefined = VdaIdentifiers.extractBizLocation(
	...
	test("Test should successfully extract VDA URI Packages License Plates", async () => {
		const id = VdaIdentifiers.extractUriPlp("urn:jaif:id:A2:1JUN499771473123456789");
	...
	test("Test should fail if VDA URI Packages License Plates length is invalid", async () => {
		const id = VdaIdentifiers.extractUriPlp("urn:jaif:id:A2:1JUN4997714732345789");
		expect(id).toBeUndefined();
	});

	test("Test should fail if VDA URI Packages License Plates application family identifier is invalid", async () => {
		const id = VdaIdentifiers.extractUriPlp("urn:jaif:id:V2:1JUN499771473123456789");
		expect(id).toBeUndefined();
	});

	test("Test should fail if VDA URI Packages License Plates package type is invalid", async () => {
		const id = VdaIdentifiers.extractUriPlp("urn:jaif:id:A2:1SUN499771473123456789");
		expect(id).toBeUndefined();
	});
});
